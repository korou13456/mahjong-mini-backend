...
async function getTapestryInventory(req, res) {
  try {
    ...
    const [transitResult] = await db.query(
      `SELECT
        ...
        COALESCE(SUM(size_60_40), 0) as size_60_40,
    ...
    const [records] = await db.query(
      `SELECT id, record_date, status, size_40_30, size_60_40, size_60_50, size_80_60, size_90_60, remark, image_urls, created_at, updated_at
       FROM tapestry_inventory_record
       WHERE ${whereClause}
    ...
    console.error("获取挂毯库存数据失败:", error);
    res.status(500).json({
      code: 500,
      message: "服务器错误",
    });
...
module.exports = getTapestryInventory;
