// 修改挂毯库存记录（仅允许修改状态为0的记录）
const db = require("../../../../config/database");

async function updateTapestryRecord(req, res) {
  try {
    const {
      id,
      record_date,
    ...
    const [existing] = await db.query(
      `SELECT * FROM tapestry_inventory_record WHERE id = ?`,
      [id]
    );
    ...
    // 更新明细记录
    await db.query(
      `UPDATE tapestry_inventory_record SET
       record_date = ?,
       status = ?,
       size_40_30 = ?, size_60_40 = ?, size_60_50 = ?, size_80_60 = ?, size_90_60 = ?,
       remark = ?,
    ...
    console.error("修改挂毯库存记录失败:", error);
    res.status(500).json({
      code: 500,
      message: "服务器错误",
    });
...
module.exports = updateTapestryRecord;
