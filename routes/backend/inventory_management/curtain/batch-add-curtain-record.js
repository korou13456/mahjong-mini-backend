...
async function batchAddCurtainRecord(req, res) {
  try {
    ...
    const [existingRecords] = await db.query(
      `SELECT batch_hash FROM curtain_inventory_record WHERE batch_hash IN (${allHashes.map(() => '?').join(',')})`,
      allHashes
    );
    const existingHashes = new Set(existingRecords.map(r => r.batch_hash));

    for (const record of records) {
      const { record_date, status, size_52_63 = 0, size_52_84 = 0, remark, image_urls } = record;
      ...
      toInsert.push([record_date, status, size_52_63 * multiplier, size_52_84 * multiplier, remark || null, image_urls ? JSON.stringify(image_urls) : null, batch_hash]);

      // 只有入库(1)和出库(2)才更新总量表，在路上(0)不更新
      if (status === 1 || status === 2) {
        inventoryUpdates.push([size_52_63 * multiplier, size_52_84 * multiplier]);
      }
    ...
    if (toInsert.length > 0) {
      ...
      await db.query(
        `INSERT INTO curtain_inventory_record (record_date, status, size_52_63, size_52_84, remark, image_urls, batch_hash) VALUES ${placeholders}`,
        flattened
      );
    }
    ...
    if (inventoryUpdates.length > 0) {
      ...
      await db.query(
        `UPDATE curtain_inventory SET size_52_63 = size_52_63 + ?, size_52_84 = size_52_84 + ?, updated_at = CURRENT_TIMESTAMP`,
        totalChange
      );
    }
    ...
    console.error("批量添加窗帘库存记录失败:", error);
    res.status(500).json({ code: 500, message: "服务器错误" });
  }
}

module.exports = batchAddCurtainRecord;
