...
async function getMousepadInventory(req, res) {
  try {
    ...
    const [records] = await db.query(
      `SELECT id, record_date, status, size_30_80, remark, image_urls, created_at, updated_at
       FROM mousepad_inventory_record
       WHERE ${whereClause}
    ...
    console.error("获取鼠标垫库存数据失败:", error);
    res.status(500).json({
      code: 500,
      message: "服务器错误",
    });
...
module.exports = getMousepadInventory;
