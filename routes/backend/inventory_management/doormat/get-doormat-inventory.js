...
async function getDoormatInventory(req, res) {
  try {
    ...
    const [inventory] = await db.query(
      `SELECT id, size_40_60, size_43_75, size_43_120, updated_at
       FROM doormat_inventory
       LIMIT 1`
    );
    ...
    const [transitResult] = await db.query(
      `SELECT
        COALESCE(SUM(size_40_60), 0) as size_40_60,
        COALESCE(SUM(size_43_75), 0) as size_43_75,
        COALESCE(SUM(size_43_120), 0) as size_43_120
       FROM doormat_inventory_record
       WHERE status = 0`
    );
    ...
    const [totalResult] = await db.query(
      `SELECT COUNT(*) as total FROM doormat_inventory_record WHERE ${whereClause}`,
      params
    );
    ...
    const [records] = await db.query(
      `SELECT id, record_date, status, size_40_60, size_43_75, size_43_120, remark, image_urls, created_at, updated_at
       FROM doormat_inventory_record
       WHERE ${whereClause}
    ...
    console.error("获取地垫库存数据失败:", error);
    res.status(500).json({
      code: 500,
      message: "服务器错误",
    });
...
module.exports = getDoormatInventory;
