...
async function batchAddTshirtRecord(req, res) {
  try {
    ...
    for (const record of records) {
      if (!record.record_date) {
        return res.json({ code: 400, message: "记录日期不能为空" });
      }
      if (record.status === undefined || record.status === null) {
        return res.json({ code: 400, message: "状态不能为空" });
      }
      if (![0, 1, 2].includes(record.status)) {
        return res.json({ code: 400, message: "状态值无效，应为 0(在路上)、1(入库)、2(出库)" });
      }
    }
    ...
    const [existingRecords] = await db.query(
      `SELECT batch_hash FROM tshirt_inventory_record WHERE batch_hash IN (${allHashes.map(() => '?').join(',')})`,
      allHashes
    );
    ...
    console.error("批量添加T恤库存记录失败:", error);
    res.status(500).json({ code: 500, message: "服务器错误" });
  }
}

module.exports = batchAddTshirtRecord;
